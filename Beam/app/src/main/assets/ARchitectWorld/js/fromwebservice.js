...
var ServerInformation = {
	POIDATA_SERVER: "https://10.12.48.154/FriendLocation",
	POIDATA_SERVER_ARG: "user="
...
//		World.markerDrawable_selected = new AR.ImageResource("assets/cloud.png");
...
// called to inject new POI data
	...
	*/
	// request POI data
	requestDataFromServer: function requestDataFromServerFn(user) {
        if (!World.initiallyLoadedData) {
        AR.logger.debug("requestDataFromServer");
		// set helper var to avoid requesting places while loading
		World.isRequestingData = true;
		World.updateStatusMessage('Requesting places from web-service');

		// server-url to JSON content provider
		var serverUrl = ServerInformation.POIDATA_SERVER + "?" + ServerInformation.POIDATA_SERVER_ARG + user;
